...
export default async function ServicesPage() {
  ...
  // Récupération des champs dynamique depuis general_remarks
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-16 sm:py-24">
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {priceLabel && (
                      <span className="shrink-0 text-xs font-bold text-violet-700 bg-violet-100 px-3 py-1.5 rounded-full whitespace-nowrap">
                        {priceLabel}
                      </span>
                    )}
                  </div>
      ...
      {/* BLOC D'INFORMATION ET REMARQUES DYNAMIQUE */}
      {hasRemarksBlock && (
        <ScrollReveal delay={(services?.length ?? 0) * 100}>
          <div className="mt-16 bg-white border border-slate-200/80 rounded-3xl p-8 sm:p-10 shadow-sm space-y-8">
            ...
            {/* Titre & Texte d'introduction issus de Supabase */}
            {(introTitle || introText) && (
              <div className="space-y-3">
                ...
                {introText && (
                  <p className="text-slate-600 text-sm sm:text-base leading-relaxed sm:pl-11">
                    {introText}
                  </p>
                )}
              </div>
            ...
            {/* Liste de points de la colonne `remarques` */}
            {remarques.length > 0 && (
